...
class SiteScraper(ConfigureSiteScraper):
    ...
    def get_chapter(self, node: Node, sections):
        chapter = node.css_first('#chr-content');
        if chapter:
    ...
    def get_titles(self, node: Node, sections: list[str]):
        chapter = node.css_first('.chr-text')
        story = node.css_first('.novel-title')
        return KeyResult(
    ...
    def get_urls(self, node: Node, sections: list[str], url: str):
        prev = node.css_first('#prev_chap');
        next = node.css_first('#next_chap');
        return UrlResult(
