...
class SiteScraper(ConfigureSiteScraper):
    ...
    def getConfiguration(self, url):
        return BasicConfiguration(
            ...
            get_title = lambda node: node.css_first('#chaptercontent header h3'),
            get_chapter = lambda node, sections: node.css_first('#chaptercontent'),
            get_buttons = lambda node: self.Object(
                prev = node.css_first('#pagination_area a[rel=prev]'),
                next = node.css_first('#pagination_area a[rel=next]'),
            ),
