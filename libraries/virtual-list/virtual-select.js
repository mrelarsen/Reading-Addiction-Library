export class VirtualList extends Element {
    ...
    render() {
        ...
        for (let index = firstIndex; index <= lastIndex; ++index) {
            ...
            if (item) list.push(this.renderItem(item, item === currentItem, selectedItems?.includes(item)));
        }
    ...
    appendElements(index, n) {
        ...
        for (let i = 0; i < n; ++i, ++index) {
            ...
            const item = this.itemAt(index);
            elements.push(this.renderItem(item, item === currentItem, selectedItems?.includes(item)));
        }
